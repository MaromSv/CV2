...
from src.utils.eval_utils import eval_rendering, eval_ate
...
class SLAM:
    ...
    def terminate(self):
        ...
        mapping_time = np.array(self.mapper.mapping_timings).mean()
        tracking_time = np.array(self.mapper.tracking_timings).mean()
        #dspo_time = np.array(self.tracker.frontend.graph.dspo_timings).mean()

        mapping_time_over_all_frames = len(self.stream) / np.array(self.mapper.mapping_timings).sum()
        tracking_time_over_all_frames = len(self.stream) / np.array(self.mapper.tracking_timings).sum()  
        #dspo_time_over_all_frames = len(self.stream) / np.array(self.tracker.frontend.graph.dspo_timings).sum()

        print(f"Average mapping time: {mapping_time}")
        print(f"Average tracking time: {tracking_time}")
        print(f"Average mapping time over all frames: {mapping_time_over_all_frames}")
        print(f"Average tracking time over all frames: {tracking_time_over_all_frames}")
        ...
        if self.cfg['tracking']['backend']['final_ba'] and self.cfg['mapping']['eval_before_final_ba']:
            ...
            if not self.only_tracking: 
                ...
                for kf_idx in self.mapper.video_idxs:
                    R, t, theta, rho = cams[kf_idx].get_virtual_extrinsics(return_gradients=False)
                    for virtual_cam in range(cams[kf_idx].n_virtual_cams):
                        traj_est_aligned.append(np.linalg.inv(gen_pose_matrix(R[virtual_cam], t[virtual_cam])))
